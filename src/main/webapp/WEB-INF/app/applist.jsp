	...
	<table class="table table-bordered table-hover definewidth m10">
		...
		<s:iterator value="pageModel.result" var="u">
			<tr>
				...
				<td>
				 <s:if test="#u.state=='000'">
				    审核中
				 </s:if>
				 <s:elseif test="#u.state=='010'">
				   审核通过
				 </s:elseif>
				  <s:elseif test="#u.state=='011'">
				    审核未通过
				  </s:elseif>
				   <s:elseif test="#u.state=='020'">
				    调度成功
				  </s:elseif>
				   <s:elseif test="#u.state=='021'">
				    调度不成功
				  </s:elseif>
				</td>
				<td>
				<a href="car/app_updatePage?id=<s:property value="#u.id" />">编辑</a>
				|<a href="car/app_delete?id=<s:property value="#u.id" />">删除</a>
				|
				<a href="car/app_audit?id=<s:property value="#u.id" />&state=010">审核通过</a>
				|
				<a href="car/app_audit?id=<s:property value="#u.id" />&state=011">审核不通过</a>
